...
df['note_id'] = df['ROW_ID'].astype(str)
...
def read_patient(name, period_length, allowed_types, eps = 0.001, dtype = 'train', return_intime = False):
    ...
    result = df[(df['SUBJECT_ID'] == subj_id) & (df['HADM_ID'] == hadm_id)
       & (pd.to_datetime(df['CHARTTIME']) >= intime) & (pd.to_datetime(df['CHARTTIME']) < intime+pd.Timedelta(hours = period_length + eps))
...
def agg_notes(notes, first = False, intime = None, timeDiff = pd.Timedelta(hours = 48)):
    ...
    if first:
        ...
        if firstgood.shape[0] > 0 and (pd.to_datetime(firstgood.iloc[0]['CHARTTIME']) - intime) <= timeDiff:
            df['note_id'] = df['note_id'].astype(str)
            note_to_take = firstgood.iloc[0]
        elif (pd.to_datetime(notes.iloc[0]['CHARTTIME']) - intime) <= timeDiff:
