...
print(args.target_col_name)

save_location = "/home/dom_dillingham/HurtfulWords/data/models/finetuned/newembed/model/"
if args.target_col_name == 'Acute and unspecified renal failure':
    model = AutoModel.from_pretrained(model_name)
else:
    model = AutoModel.from_pretrained(save_location)
tokenizer = AutoTokenizer.from_pretrained(model_name)

args.max_num_epochs = 5
args.freeze_bert = False
args.gridsearch_classifier = False
...
if args.gridsearch_c:
    ...
    c_grid = [0.001, 0.05, 0.1, 0.2, 0.5, 0.7, 1, 1.2, 1.5, 2, 3, 5, 10]
else:
    c_grid = [2]
...
def get_embs(generator):
    ...
    with torch.no_grad():
        for input_ids, input_mask, segment_ids, y, group, guid, other_vars in generator:
            ...
            hidden_states, _ = model(input_ids, token_type_ids = segment_ids, attention_mask = input_mask,return_dict=False)

            #bert_out = extract_embeddings(hidden_states, 'last')
            bert_out = hidden_states[-1]
...
if not args.freeze_bert:
    ...
    output_model_file = os.path.join(save_location, WEIGHTS_NAME)
    output_config_file = os.path.join(save_location, CONFIG_NAME)
    torch.save(model_to_save.state_dict(), output_model_file)
    model_to_save.config.to_json_file(output_config_file)
    tokenizer.save_vocabulary(save_location)
